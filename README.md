# 🚀 Sing-box 极限轻量双协议一键脚本

针对 **72MB 极限小内存服务器（如 Alpine Linux / NAT 小鸡）** 深度定制优化的 Sing-box 双协议一键部署脚本。

* ✂️ **极致瘦身**：彻底移除 `jq` 等高内存依赖，改用原生 Shell 变量处理，杜绝安装期 `Killed` 报错。
...
请复制下方命令，在你的服务器终端（SSH）中整行粘贴并回车执行：
